...
/**
 * Capabilities for an ECP event source (declared during initialization).
 */
export const EventSourceCapabilitiesSchema = z.object({
    eventTypes: z.array(z.string()).optional(),
    contextEnrichment: z.boolean().optional(),
    actionExecution: z.boolean().optional()
});

/**
 * Capabilities for an ECP agent (declared during initialization).
 */
...
export const ClientCapabilitiesSchema = z.object({
    ...
    /**
     * Present if the client is an ECP event source that can emit events.
     */
    events: EventSourceCapabilitiesSchema.optional()
});
...
export const ServerCapabilitiesSchema = z.object({
    ...
    /**
     * Present if the server is an ECP agent that can receive events.
     */
    eventSubscriptions: AgentCapabilitiesSchema.optional()
});
...
/* ======================== */
/* ECP (Event Context Protocol) */
/* ======================== */

/**
 * The intent classification for a context frame event.
 */
export const EventIntentSchema = z.enum(['requires_action', 'informational', 'requires_approval', 'context_update']);

/**
 * The urgency level for a context frame event.
 */
export const EventUrgencySchema = z.enum(['critical', 'high', 'normal', 'low']);

/**
 * A suggested tool that the agent may use when handling this event.
 */
...
/**
 * Inline context data attached to a context frame.
 */
export const FrameContextSchema = z.object({
    resources: z.array(z.looseObject({ uri: z.string(), mimeType: z.string().optional(), text: z.string().optional() })).optional(),
    variables: z.record(z.string(), z.unknown()).optional(),
    suggestedTools: z.array(SuggestedToolSchema).optional()
});

/**
 * A semantic, context-rich event payload emitted by an event source.
 */
export const ContextFrameSchema = z.object({
    ...
    context: FrameContextSchema.optional(),
...
/**
 * An action taken by the agent in response to an event.
 */
...
/**
 * The result of processing a context frame event.
 */
export const ActionResultSchema = ResultSchema.extend({
    frameId: z.string(),
    actionsTaken: z.array(ActionTakenSchema).optional(),
    status: z.enum(['handled', 'deferred', 'ignored', 'error']),
    summary: z.string().optional()
});

/**
 * An event type descriptor published by an event source.
 */
export const EventTypeDescriptorSchema = z.object({
    eventType: z.string(),
    description: z.string().optional(),
...
/**
 * Emits a context frame from the event source to the agent.
 * Direction: EventSource (client) → AgentServer (server)
 */
export const EmitEventRequestParamsSchema = BaseRequestParamsSchema.extend({
    frame: ContextFrameSchema
});

export const EmitEventRequestSchema = RequestSchema.extend({
    method: z.literal('events/emit'),
    params: EmitEventRequestParamsSchema
});

/**
 * Subscribes to events from the event source.
 * Direction: AgentServer (server) → EventSource (client)
 */
export const SubscribeEventsRequestParamsSchema = BaseRequestParamsSchema.extend({
    patterns: z.array(z.string())
});

export const SubscribeEventsRequestSchema = RequestSchema.extend({
    method: z.literal('events/subscribe'),
    params: SubscribeEventsRequestParamsSchema
});

export const SubscribeEventsResultSchema = ResultSchema.extend({
    subscribed: z.array(z.string())
});

/**
 * Lists event types available from the event source.
 * Direction: AgentServer (server) → EventSource (client)
 */
export const ListEventTypesRequestSchema = RequestSchema.extend({
    method: z.literal('events/list_types'),
    params: BaseRequestParamsSchema.optional()
});

export const ListEventTypesResultSchema = ResultSchema.extend({
    eventTypes: z.array(EventTypeDescriptorSchema)
});

/**
 * Requests additional context from the event source.
 * Direction: AgentServer (server) → EventSource (client)
 */
export const EnrichContextRequestParamsSchema = BaseRequestParamsSchema.extend({
    frameId: z.string(),
    fields: z.array(z.string()).optional()
});

export const EnrichContextRequestSchema = RequestSchema.extend({
    method: z.literal('context/enrich'),
    params: EnrichContextRequestParamsSchema
});

export const EnrichContextResultSchema = ResultSchema.extend({
    frameId: z.string(),
    context: FrameContextSchema
});

/**
 * Requests the event source to execute an action.
 * Direction: AgentServer (server) → EventSource (client)
 */
export const ExecuteActionRequestParamsSchema = BaseRequestParamsSchema.extend({
    frameId: z.string(),
    action: z.string(),
    params: z.record(z.string(), z.unknown()).optional()
});

export const ExecuteActionRequestSchema = RequestSchema.extend({
    method: z.literal('actions/execute'),
    params: ExecuteActionRequestParamsSchema
});

export const ExecuteActionResultSchema = ResultSchema.extend({
    success: z.boolean(),
    result: z.unknown().optional(),
...
/**
 * Notification that the set of available event types has changed.
 */
export const EventTypesChangedNotificationSchema = NotificationSchema.extend({
    method: z.literal('notifications/events/types_changed'),
    params: NotificationsParamsSchema.optional()
});
...
export const ClientNotificationSchema = z.union([
    ...
    EventTypesChangedNotificationSchema
]);
...
export const ServerRequestSchema = z.union([
    ...
    ListEventTypesRequestSchema,
    EnrichContextRequestSchema,
...
/* ECP */
export type EventIntent = Infer<typeof EventIntentSchema>;
export type EventUrgency = Infer<typeof EventUrgencySchema>;
export type SuggestedTool = Infer<typeof SuggestedToolSchema>;
export type FrameContext = Infer<typeof FrameContextSchema>;
export type ContextFrame = Infer<typeof ContextFrameSchema>;
export type ActionTaken = Infer<typeof ActionTakenSchema>;
export type ActionResult = Infer<typeof ActionResultSchema>;
export type EventSourceCapabilities = Infer<typeof EventSourceCapabilitiesSchema>;
export type AgentCapabilities = Infer<typeof AgentCapabilitiesSchema>;
export type EventTypeDescriptor = Infer<typeof EventTypeDescriptorSchema>;
export type EmitEventRequestParams = Infer<typeof EmitEventRequestParamsSchema>;
export type EmitEventRequest = Infer<typeof EmitEventRequestSchema>;
export type SubscribeEventsRequestParams = Infer<typeof SubscribeEventsRequestParamsSchema>;
export type SubscribeEventsRequest = Infer<typeof SubscribeEventsRequestSchema>;
export type SubscribeEventsResult = Infer<typeof SubscribeEventsResultSchema>;
export type ListEventTypesRequest = Infer<typeof ListEventTypesRequestSchema>;
export type ListEventTypesResult = Infer<typeof ListEventTypesResultSchema>;
export type EnrichContextRequestParams = Infer<typeof EnrichContextRequestParamsSchema>;
export type EnrichContextRequest = Infer<typeof EnrichContextRequestSchema>;
export type EnrichContextResult = Infer<typeof EnrichContextResultSchema>;
export type ExecuteActionRequestParams = Infer<typeof ExecuteActionRequestParamsSchema>;
export type ExecuteActionRequest = Infer<typeof ExecuteActionRequestSchema>;
export type ExecuteActionResult = Infer<typeof ExecuteActionResultSchema>;
export type EventTypesChangedNotification = Infer<typeof EventTypesChangedNotificationSchema>;
...
export type ResultTypeMap = {
    ...
    'events/emit': ActionResult;
    'events/subscribe': SubscribeEventsResult;
    'events/list_types': ListEventTypesResult;
    'context/enrich': EnrichContextResult;
    'actions/execute': ExecuteActionResult;
};
